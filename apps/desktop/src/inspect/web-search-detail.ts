...
function operationDetail(operation: WebSearchOperation): string | undefined {
  switch (operation.kind) {
    ...
    case "open":
      return operation.line_number === undefined
        ? undefined
        : `Starting at line ${operation.line_number}`;
    case "click":
...
export class WebSearchDetail extends LitElement {
  ...
  protected updated(changed_properties: PropertyValues<this>) {
    if (
      changed_properties.has("request_url") ||
      changed_properties.has("response_url")
    ) {
      void this.load();
    }
  }
  ...
  private async load() {
    ...
    try {
      const [request, response] = await Promise.all([
        readInspector<RequestPayload>(request_url, controller.signal),
        readInspector<RequestPayload>(response_url, controller.signal),
      ]);
      if (
        this.load_controller !== controller ||
      ...
      if (
        request.field !== "inbound_req_body" ||
        response.field !== "inbound_resp_body"
      ) {
        throw new Error(
          "Search payload response did not match the requested fields",
        );
      }
      ...
      this.error_message =
        error instanceof Error ? error.message : "Unable to load web search";
    } finally {
  ...
  render() {
    if (this.load_state === "loading" || this.load_state === "idle") {
      return html`
        <section
          class="web-search-inspection web-search-state"
    ...
    if (this.load_state === "error") {
      return html`
        <section
          class="web-search-inspection web-search-state error-state"
          aria-label="Web search"
          role="alert"
        >
          <div>
            <strong>Web search could not be loaded</strong
    ...
    return html`
      <section class="web-search-inspection" aria-label="Web search">
          ...
          <div class="web-search-metrics">
            ...
            ${
              inspection.response_length
                ? html`<span
                    ><strong>${inspection.response_length}</strong>
                    response</span
                  >`
                : nothing
            }
            ${
              inspection.encrypted_output_bytes !== undefined
                ? html`<span title="Decoded encrypted payload size"
                    ><strong
                      >${formatBytes(inspection.encrypted_output_bytes)}</strong
                    >
                    encrypted</span
                  >`
                : nothing
        ...
        <div class="web-search-operations">
          ${
            inspection.operations.length === 0
              ? html`<p class="web-search-empty">
                  No supported web operation was persisted.
                </p>`
              : inspection.operations.map((operation, index) => {
                  const detail = operationDetail(operation);
                  const href =
                    operation.kind === "open"
                      ? safeHttpUrl(operation.value)
                      : undefined;
                  return html`
                    <article>
                      <span class="web-search-operation-index"
                        >${index + 1}</span
                      >
                      <div>
                        <span class="web-search-operation-kind"
                          >${operationKindLabel(operation)}</span
                        >
                        ${
                          href
                            ? html`<a
        ...
        <dl class="web-search-settings">
          <div>
            <dt>Caller</dt>
            <dd>${inspection.allowed_callers.join(", ") || "—"}</dd>
          </div>
          <div>
            <dt>External web access</dt>
            <dd>
              ${inspection.external_web_access === undefined ? "—" : String(inspection.external_web_access)}
            </dd>
          </div>
        </dl>
                          ...
                          ${
                            href
                              ? html`<a
                                ...
                                >`
                              : html`<strong
                                  >${result.title ?? result.url ?? "Untitled result"}</strong
                                >`
                          }
        ...
        <div class="payload-stack web-search-payloads">
          ${
            inspection.output
              ? html`<payload-panel
                  label="Synthesized search output"
                  .value=${inspection.output}
                ></payload-panel>`
              : nothing
          }
          ${
            inspection.prompt
              ? html`<payload-panel
                  label="Prompt context sent to search"
                  .value=${inspection.prompt}
                ></payload-panel>`
              : nothing
          }
        </div>
