...
const inspection = inspectWebSearch(
  {
    ...
    commands: {
      search_query: [
        { q: "rust sqlite viewer", domains: ["docs.rs"], recency: 30 },
      ],
...
assertEqual(
  webSearchOperationSummary(open_inspection.operations),
  "2 page opens",
  "open requests need an accurate heading",
);
assertEqual(
  webSearchOperationSummary([]),
  "No operations",
  "missing commands should not be described as zero queries",
);

assertEqual(
  decodedBase64ByteLength("AQID"),
  3,
  "unpadded base64 should have a decoded size",
);
assertEqual(
  decodedBase64ByteLength("gA-_"),
  3,
  "URL-safe base64 should have a decoded size",
);
assertEqual(
  decodedBase64ByteLength("not base64!"),
  undefined,
  "invalid base64 should not have a decoded size",
);
assertEqual(
  isCodexWebSearchEndpoint(
    "https://chatgpt.com/backend-api/codex/alpha/search?feature=1",
  ),
  true,
  "full search URLs should match",
);
assertEqual(
  isCodexWebSearchEndpoint("/backend-api/codex/responses"),
  false,
  "other Codex URLs should not match",
);
assertEqual(
  safeHttpUrl("https://example.com/result"),
  "https://example.com/result",
  "HTTPS result URLs should be links",
);
assertEqual(
  safeHttpUrl("javascript:alert(1)"),
  undefined,
  "unsafe result URLs should not be links",
);
