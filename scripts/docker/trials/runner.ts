...
import { resolveAgent, type TrialCase, type TrialResult } from "./agents";
import { checked, directNetworkEnv, type ContainerEngine } from "./engine";
import { resolveMode } from "./modes";
import { validateInputs, type TrialSuite } from "./suite";
...
export type CaseReport = { trial: TrialCase; result: TrialResult; exit_code: number | null; timed_out: boolean };
...
export function gatewayMounts(suite: TrialSuite, volume_name: string): string[] {
  return [
...
export async function runSuite(engine: ContainerEngine, suite: TrialSuite, options: RunOptions): Promise<RunReport> {
  validateInputs(suite);
  const prepared = suite.cases.map((trial) => {
    const adapter = resolveAgent(trial.agent);
    const mode = resolveMode(trial.mode);
    return { trial, adapter, mode, input: adapter.prepare(trial, { router_url: suite.router_url }) };
  });
  ...
  const run_id = `tokn-trial-${randomUUID()}`;
  ...
  try {
    ...
    if (!api_key) throw new Error("Unable to create a private trial API key");
    ...
    for (const { trial, adapter, mode, input } of prepared) {
      options.signal?.throwIfAborted();
      note(`Running ${trial.id}`);
      const case_dir = join(output_dir, trial.id);
      ...
      const agent_name = `${run_id}-${trial.id}`;
      ...
      await checked(engine, [
        ...
        "--env", "TOKN_TRIAL_API_KEY", ...bind(case_dir, "/trial"), "--tmpfs", "/workspace:rw", "--workdir", input.working_dir,
        suite.agent_images[adapter.id] ?? adapter.image, ...input.command,
      ], { signal: options.signal, env: { TOKN_TRIAL_API_KEY: api_key } });
      ...
      else if (waited.exit_code !== 0) throw new Error(`Failed to wait for ${trial.id}`);
      ...
      if (exit_code !== null && (!/^[0-9]+$/.test(exit_status) || !Number.isSafeInteger(exit_code) || exit_code > 255)) {
        throw new Error(`Invalid container exit status for ${trial.id}`);
      }
      const logs = await engine.run(["logs", agent_name]);
      if (logs.exit_code !== 0 || logs.timed_out) throw new Error(`Unable to collect logs for ${trial.id}`);
      writePrivate(join(case_dir, "stdout.jsonl"), logs.stdout);
      writePrivate(join(case_dir, "stderr.log"), logs.stderr);
      const result = adapter.evaluate(trial, input, { ...logs, exit_code, timed_out: waited.timed_out });
      report.cases.push({ trial, result, exit_code, timed_out: waited.timed_out });
      note(`${trial.id}: ${result.success ? "PASS" : `FAIL (${result.error})`}`);
