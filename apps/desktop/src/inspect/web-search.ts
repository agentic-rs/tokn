...
function operationFromValue(
  kind: string,
  value: unknown,
): WebSearchOperation | undefined {
  ...
  switch (kind) {
    ...
    case "open": {
      ...
      return ref_id
        ? { kind, value: ref_id, line_number: finiteNumber(item?.lineno) }
        : undefined;
    }
    case "click": {
      const ref_id = stringValue(item?.ref_id);
      const link_id = finiteNumber(item?.id);
      return ref_id && link_id !== undefined
        ? { kind, value: ref_id, link_id }
        : undefined;
    }
    case "find": {
      const ref_id = stringValue(item?.ref_id);
...
export function webSearchOperationSummary(
  operations: WebSearchOperation[],
): string {
...
function resultFromValue(value: unknown): WebSearchResult | undefined {
  ...
  return Object.values(result).some((field) => field !== undefined)
    ? result
    : undefined;
}
...
export function inspectWebSearch(
  request: unknown,
  response: unknown,
): WebSearchInspection {
  ...
  const result_values = Array.isArray(response_record?.results)
    ? response_record.results
    : [];
  ...
  return {
    ...
    external_web_access:
      typeof settings?.external_web_access === "boolean"
    ...
    encrypted_output_bytes: encrypted_output
      ? decodedBase64ByteLength(encrypted_output)
      : undefined,
  };
}
...
export function isCodexWebSearchEndpoint(value: unknown): boolean {
  ...
  try {
    return (
      new URL(value, "http://localhost").pathname === CODEX_WEB_SEARCH_ENDPOINT
    );
  } catch {
