  ...
  for (const start of nodes) {
    ...
    while (current && !resolved.has(current.node_id)) {
      ...
      current = current.parent_node_id
        ? node_by_id.get(current.parent_node_id)
        : undefined;
    }
...
function compareNodes(
  left: SessionNodeSummary,
  right: SessionNodeSummary,
  head_path: Set<string>,
): number {
  const head_order =
    Number(head_path.has(right.node_id)) - Number(head_path.has(left.node_id));
  if (head_order !== 0) {
    return head_order;
  }
  if (left.ts !== right.ts) {
...
function collectHeadPath(
  ...
  const head = [...nodes]
    ...
    .sort(
      (left, right) =>
        right.ts - left.ts || left.node_id.localeCompare(right.node_id),
    )[0];
...
function appendPostorder(
  ...
  const stack: Array<{ node: SessionNodeSummary; next_child: number }> = [
    { node: root, next_child: 0 },
  ];
...
function orderedNodes(
  ...
  const compare = (left: SessionNodeSummary, right: SessionNodeSummary) =>
    compareNodes(left, right, head_path);
...
function buildRows(
  ...
  for (const node of nodes) {
    ...
    if (parent_id && !processed.has(parent_id)) {
      const existing_parent_lane = lanes.findIndex(
        (lane_id, index) => index !== node_lane && lane_id === parent_id,
      );
      if (existing_parent_lane === -1) {
        ...
        parent_lane =
          existing_parent_lane - Number(node_lane < existing_parent_lane);
      }
    } else {
      if (parent_id && processed.has(parent_id)) {
        cycle_node_ids.add(node.node_id);
    ...
    rows.push({
      ...
      starts_here,
      connections,
      bottom_lane_is_active: bottom_lanes.map((node_id) =>
        head_path.has(node_id),
      ),
      child_count: children_by_id.get(node.node_id)?.length ?? 0,
      parent_is_missing: Boolean(
        parent_id && missing_parent_ids.has(parent_id),
      ),
...
export function buildSessionTree(
  ...
  const children_by_id = new Map(
    unique_nodes.map((node) => [node.node_id, [] as SessionNodeSummary[]]),
  );
  ...
  for (const node of unique_nodes) {
    ...
    if (
      node_by_id.has(parent_id) &&
      !(cycle_node_ids.has(node.node_id) && cycle_node_ids.has(parent_id))
    ) {
  ...
  return {
    ...
    remaining_lanes: row_model.remaining_lanes.filter((lane_id) =>
      missing_parent_ids.has(lane_id),
    ),
