...
export function requestKey(
  request: Pick<RequestSummary, "day" | "row_id">,
): string {
...
export function requestOutcome(request: RequestSummary): RequestOutcome {
  ...
  const status =
    request.inbound_resp_status ??
    request.outbound_resp_status ??
    request.status;
  if (status === null) {
    return {
      label: "—",
      tone: "neutral",
      title: "No response status persisted",
    };
  }
  const source =
    request.inbound_resp_status !== null
      ? "Client response"
...
export function sessionOutcome(session: SessionSummary): RequestOutcome {
  ...
  if (status === null) {
    return {
      label: "—",
      tone: "neutral",
      title: "No status stored for the current session head",
    };
  }
  if (status >= 400) {
    return {
      label: String(status),
      tone: "error",
      title: `Current head status: ${status}`,
    };
  }
  if (status >= 300) {
    return {
