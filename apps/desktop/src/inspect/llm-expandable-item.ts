...
export class LlmExpandableItem extends LitElement {
  ...
  private async load() {
    ...
    this.error_message = undefined;
    try {
      const detail = await readInspector<LlmItemDetail>(
        load_url,
        controller.signal,
      );
      const expected_index = Number(
        new URL(load_url, window.location.origin).searchParams.get("index"),
      );
      if (this.load_controller !== controller || this.load_url !== load_url) {
        return;
      }
      if (
        !Number.isInteger(expected_index) ||
        detail.index !== expected_index
      ) {
  ...
  render() {
    return html`
      <details
        class="llm-expandable-item"
        ?open=${this.open}
        @toggle=${this.toggle}
      >
        <summary>
          <span class="llm-expandable-chevron" aria-hidden="true">›</span>
          <span class="llm-expandable-heading"
            ><strong>${this.title}</strong><small>${this.meta}</small></span
          >
          <span class="llm-expandable-preview"
            >${this.preview ?? "Non-text content"}</span
          >
        ...
        <div class="llm-expandable-content">
          ${
            this.load_state === "loading"
              ? html`<div class="llm-item-loading">
                  <span class="spinner" aria-hidden="true"></span>Loading full
                  content…
                </div>`
              : this.load_state === "error"
                ? html`<div class="llm-item-error" role="alert">
